...
interface TripsContextValue {
  trips: Trip[];
  isLoading: boolean;
  saveTrip: (trip: Omit<Trip, 'id' | 'tripNumber' | 'createdAt'>) => Promise<Trip>;
  updateTrip: (id: string, changes: Partial<Omit<Trip, 'id' | 'tripNumber' | 'createdAt'>>) => Promise<void>;
...
export function TripsProvider({ children }: { children: ReactNode }) {
  ...
  const deleteTrip = useCallback(async (id: string) => {
    ...
    // Remove the deleted trip, then renumber all remaining trips 1, 2, 3…
    // in chronological order so TripCard never shows gaps like "#1, #3".
    const remaining = trips.filter((t) => t.id !== id);
    const renumbered = [...remaining]
      .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())
      .map((t, idx) => ({ ...t, tripNumber: idx + 1 }));

    setTrips(renumbered);
    await persistAndSyncTrips(pid, renumbered, { deletedTripId: id });
  }, [trips, resolveActiveParticipantId, persistAndSyncTrips]);
  ...
  return (
    <TripsContext.Provider value={{ trips, isLoading, saveTrip, updateTrip, deleteTrip, clearAllTrips }}>
      {children}
...
export function useTripsContext(): TripsContextValue {
  ...
  if (!ctx) throw new Error('useTripsContext must be used inside <TripsProvider>');
  return ctx;
}
